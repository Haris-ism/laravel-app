...
<form wire:submit="stageEdit({{ $post->id }})" class="space-y-5">
    ...
    <div>
        <label for="edit-title-{{ $post->id }}" class="block text-sm font-semibold text-gray-700 mb-1.5">Title</label>
        <input id="edit-title-{{ $post->id }}" type="text" wire:model="edit.title.{{ $post->id }}"
    ...
    <div>
        <label for="edit-content-{{ $post->id }}" class="block text-sm font-semibold text-gray-700 mb-1.5">Content</label>
        <textarea id="edit-content-{{ $post->id }}" rows="8" wire:model="edit.content.{{ $post->id }}"
