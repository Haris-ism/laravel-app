...
むずかしい「本格的なドメイン駆動開発（DDD）」は**あえてやりません**。実務でよく使われる**軽量版**を目標にします。

| | 本格DDD（重い・まれ） | 軽量版＝この研修の目標 |
|---|---|---|
| データの扱い | 専用の設計をゼロから作る | **Eloquent（Laravel標準）をそのまま使う** |
| 1つの仕事 | 専用クラスに分ける | **Action（ユースケース）に分ける** |
| フォルダ | 領域ごとに厳密に分割 | **機能ごとにフォルダを分ける程度** |
